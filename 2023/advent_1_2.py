...
def find_calibration_value(line):
    # Use a regex to find all digits in the line
    pattern = r"(?=(\d|one|two|three|four|five|six|seven|eight|nine))"
    digits = re.findall(pattern, line)

    # Get the first digit and last digit
    first_digit = digits[0]
    last_digit = digits[-1]

    # Convert word digits to numbers using the dictionary
    if first_digit in word_to_digit:
        first_digit = word_to_digit[first_digit]
    if last_digit in word_to_digit:
        last_digit = word_to_digit[last_digit]
...
def calculate_total_calibration(text):
...
total_calibration_value = calculate_total_calibration(lines)
