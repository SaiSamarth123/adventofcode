...
def sum_adjacent_part_numbers(puzzle_input):
    lines = puzzle_input.strip().split("\n")

    # Identify all coordinates adjacent to symbols
    symbol_adjacent = {
        (r, c)
        for i, line in enumerate(lines)
        for m in re.finditer(r"[^.\d]", line)
        for r in range(i - 1, i + 2)
        for c in range(m.start() - 1, m.start() + 2)
    }

    # Sum part numbers if they are adjacent to any symbol
    part_num_sum = sum(
        int(m.group())
        for i, line in enumerate(lines)
        for m in re.finditer(r"\d+", line)
...
sum = sum_adjacent_part_numbers(input_str)
